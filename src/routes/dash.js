...
r.get('/dashboard', requireInternal, async (_req, res) => {
  await refreshAlerts();
  const kpi = {};
  kpi.pipeline_value = (await ONE(`SELECT COALESCE(SUM(total),0) v FROM quotations WHERE status IN ('draft','pending_manager','pending_finance','sent','negotiating','returned')`)).v;
  kpi.pending_approvals = (await ONE(`SELECT COUNT(*) c FROM quotations WHERE status IN ('pending_manager','pending_finance')`)).c;
  kpi.confirmed_value = (await ONE(`SELECT COALESCE(SUM(total),0) v FROM quotations WHERE status IN ('confirmed','fulfilling','fulfilled')`)).v;
  kpi.open_invoices = await ONE(`SELECT COUNT(*) c, COALESCE(SUM(amount),0) v FROM invoices WHERE status='open' AND kind!='credit_note'`);
  kpi.paid_value = (await ONE(`SELECT COALESCE(SUM(amount),0) v FROM invoices WHERE status='paid'`)).v;
  ...
  kpi.quotes_by_status = await Q(`SELECT status, COUNT(*) c, COALESCE(SUM(total),0) v FROM quotations GROUP BY status`);
  kpi.monthly = await Q(`SELECT substr(confirmed_at,1,7) m, COUNT(*) c, COALESCE(SUM(total),0) v, COALESCE(AVG(margin_pct),0) margin
    FROM quotations WHERE confirmed_at IS NOT NULL GROUP BY substr(confirmed_at,1,7) ORDER BY m`);
  kpi.top_products = await Q(`SELECT l.description, SUM(l.qty) qty, SUM(l.qty*l.unit_price*(1-l.discount_pct/100)) revenue
    FROM quotation_lines l JOIN quotations q ON q.id=l.quotation_id WHERE q.status IN ('confirmed','fulfilling','fulfilled')
    GROUP BY l.description ORDER BY revenue DESC LIMIT 6`);
  kpi.recurring_mrr = (await ONE(`SELECT COALESCE(SUM(CASE l.billing_period WHEN 'monthly' THEN l.qty*l.unit_price*(1-l.discount_pct/100)
    ...
    FROM quotation_lines l JOIN quotations q ON q.id=l.quotation_id WHERE l.line_type='subscription' AND q.status IN ('confirmed','fulfilling','fulfilled')`)).v;
...
async function reportRows(q) {
  ...
  const rows = await Q(`SELECT q.number, q.status, q.approval_level, q.currency, q.subtotal, q.discount_total, q.tax_total, q.total, q.cost_total,
    ...
    WHERE ${where.join(' AND ')} ORDER BY q.confirmed_at DESC NULLS LAST`, params);
  const totals = {
    count: rows.length,
